 ...
 *
 * DebugStream provides a way to output debug information conditionally based on a CLI flag.
 * It overloads the `<<` operator to enable usage similar to standard output streams.
 * DebugStream checks the 'debug_mode' flag before outputting any message. If 'debug_mode' is false,
 * it does nothing, which will silence the debug output. Otherwise, it outputs to the console.
 */
class DebugStream;
